export function TypewriterName({ firstName, lastName }: { firstName: string; lastName: string }) {
  return (
    <h1 className="mt-5 font-black uppercase leading-[0.88] tracking-[-0.055em]">
      ...
      <span className="mt-2 block text-[clamp(3.2rem,5.8vw,5.9rem)]">
        {Array.from(lastName).map((char, index) => (
          <span
            key={index}
            className="name-char inline-block bg-gradient-to-b from-[#8aa0ff] via-[#6685ff] to-[#4569ff] bg-clip-text text-transparent"
            style={{ animationDelay: `${0.3 + firstName.length * 0.08 + index * 0.08}s` }}
          >
            {char === " " ? "\u00A0" : char}
          </span>
        ))}
